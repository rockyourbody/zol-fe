				...
				if(data && jQuery.isArray(data)){
					...
					jQuery.each(data,function(index,item){
						complete += '<li><form action="http://order.zol.com/index.php?c=ConfirmOrder" method="POST" data-totalPrice="0" target="_blank">\
							...
							<input type="hidden" name="cartId[]" value="'+ item.goodsInfo[0].cartId +'" />\
			...
			UserCoupon : function(data){
				...
				if(data && jQuery.isArray(data)){
					...
					jQuery.each(data,function(index,item){
						...
						complete += '>';
						complete += '<a href="'+item.shopUrl+'" class="btns">立即使用</a>'
							 +'<div class="amount">&yen;<span>'+item.couponPrice+'</span>'
							 +'<div class="usage"><p class="unit">元优惠券</p><p>'+item.couponRange+'</p></div></div>'
                                                         +'<div class="aging">有限期：'+item.startTime+'-'+item.stopTime+'</div>'
							 +'<div class="stores"><a href="'+item.shopUrl+'">'+item.merchantName+'</a></div>';
						//complete += '<div class="coupons11"><div class="amount">&yen;<span>'+item.couponPrice+'</span></div><div class="aging">'+item.startTime+'~'+item.stopTime+'</div><div class="usage"><span>'+item.couponRange+'</span>|<span>'+item.couponConditions+'</span></div></div><div class="store-links"><a href="'+item.shopUrl+'">'+item.merchantName+'&nbsp;&nbsp;&nbsp;&nbsp;进店看看&gt;</a></div></li>';
					});
					return complete;
				}
			}
